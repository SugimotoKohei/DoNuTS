...
def select_directory(initial_path):
    ...
    tk = tkinter.Tk()
    tk.withdraw()
    dicom_directory = filedialog.askdirectory(initialdir = initial_path, title='DICOMファイルが含まれるフォルダを選択')
    ...
    for num, rdsr in enumerate(rdsr_files):
        for eve in range(int(events[num])):
            for name in rdsr_header_col_names:
                try:
                    tmp_header_dictionary[name].append(str(getattr(rdsr, name)))
                except:
...
def extract_information_from_PET(PET):
    ...
    for p in PET:
        ...
        pet_tmp_data.append(p.RadiopharmaceuticalInformationSequence[0].RadionuclideTotalDose)
    ...
    if int(len(rdsr_files)) == 0:
        ...
        for num, r in enumerate(rdsr_files):
            for e in range(int(events_list[num])):
                CDLPT_dictionary['CTDoseLengthProductTotal'].append(extract_CT_Dose_Length_Product_Total(r))


        # rdsrのヘッダーから情報を抽出し辞書にまとめる
        # 取得するデータ一覧
        rdsr_header_col_names = ['ManufacturerModelName', 'PatientID', 'StudyDate', 'PatientName', 'StudyDescription', 'PatientBirthDate', 'PatientSex', 'PatientAge', 'PatientSize', 'PatientWeight']
        # 取得するデータを入れるdictionaryを作成
        rdsr_header_dictionary = {col:[] for col in rdsr_header_col_names}
        tmp_dictionary = extract_data_from_rdsr_header(rdsr_header_col_names, rdsr_files, events_list)
